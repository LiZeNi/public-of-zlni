...
class GroupQueryAttention(nn.Module):
    def __init__(self,embedd_dim,num_heads,num_gruops):
        super().__init__()
        self.embedd_dim=embedd_dim
        self.num_heads=num_heads
        self.num_groups=num_gruops
    ...
    def forward(self,query,key,value):
        ...
        K=self.K_linear(key).view(batch,-1,self.num_groups,self.head_dim).transpose(1,2).repeat(1,int(self.num_heads/self.num_groups),1,1)
        V=self.V_linear(value).view(batch,-1,self.num_groups,self.head_dim).transpose(1,2).repeat(1,int(self.num_heads/self.num_groups),1,1)
...
if  __name__=="__main__":
    ...
    model=GroupQueryAttention(embedd_dim,num_heads,num_groups)
