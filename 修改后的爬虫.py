...
def data_deal(stars, need_num, times):
    ...
    mode_star = df['stars'].mode()[0]  # 计算评分的众数
    threshold = 3  # 设置范围，允许与众数的差别在 ±3 以内
    # 去除与众数差别过大的不良数据
    filtered_df = df[(df['stars'] >= mode_star - threshold) & (df['stars'] <= mode_star + threshold)]
    first_n_values = filtered_df['stars'].head(need_num[0])
    return first_n_values.mean()


def get_points(df,min_len_text):
    result = []
        ...
        for k, kind_content in enumerate(kind):
            ...
            while True:
                ...
                for num,container in enumerate(user_container):
                    ...
                    #忽略字数过少的不良数据
                    if target_text.group(1).strip() and len(target_text.group(1).strip())>min_len_text:
...
if __name__ == '__main__':
    ...
    result = get_points(df,min_len_text=10)
